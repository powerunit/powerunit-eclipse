 ...
 *
 */
public class PowerunitExtensionsQuickFixProcessor implements IQuickFixProcessor {
	...
	public IJavaCompletionProposal[] getCorrections(
		...
		for (int i = 0; i < locations.length; i++) {
			...
			if (IProblem.UndefinedType == id) {
				res = getAddPowerunitExtensionsToBuildPathProposals(context,
						problem, res);
			}
		}
	...
	private ArrayList<IJavaCompletionProposal> getAddPowerunitExtensionsToBuildPathProposals(
		...
		try {
			...
			String s = unit.getBuffer().getText(location.getOffset(),
					location.getLength());
			if ("ProvideMatchers".equals(s)) { //$NON-NLS-1$ //$NON-NLS-2$

				String qualifiedName = "ch.powerunit.extensions.matchers." + s; //$NON-NLS-1$
				...
				for (int i = 0; i < fixProposals.length; i++) {
					...
					proposals
							.add(new PowerUnitExtensionsClasspathFixCorrectionProposal(
									javaProject, fixProposals[i],
									getImportRewrite(context.getASTRoot(),
											qualifiedName)));
				}
			}
	...
	private static class PowerUnitExtensionsClasspathFixCorrectionProposal
		...
		public PowerUnitExtensionsClasspathFixCorrectionProposal(
